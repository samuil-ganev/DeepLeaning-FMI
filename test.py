...
assert len(
    set(a1.generateEdits("тест")) - set(["тест"])) == 287, "Броят на елементарните редакции \"тест\"  следва да е 287"
...
def test_bestAlignment(s1, s2, alignment):
    ...
    for u, v in alignment:
        if len(u) == 1 and len(v) == 1:
            if u != v: w += 1
        elif len(u) == 0 and len(v) == 1:
            w += 1
        elif len(u) == 1 and len(v) == 0:
            w += 1
        elif len(u) == 2 and len(v) == 2 and u[0] == v[1] and u[1] == v[0]:
            w += 1
        else:
            w = None
            break
...
for s1, s2, d in zip(L1, L2, C):
    assert test_bestAlignment(s1, s2,
                              a1.bestAlignment(s1, s2)) == d, "Грешно минимално подравняване между '{}' и '{}'".format(
        s1, s2)
...
fullSentCorpus = [[langmodel.startToken] + [w.lower() for w in sent] + [langmodel.endToken] for sent in
                  myCorpus.sents()]
print('Готово.')
...
assert a1.correctSpelling("фатовно по футбол", M2, weights,
                          1.0) == 'правителството ще успее', "Коригираната заявка следва да е 'световно по футбол'"
